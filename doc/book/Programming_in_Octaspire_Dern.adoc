...
Documentation for Octaspire Dern programming language version 0.104.0
...
Currently it is tested with *gcc*, *clang* and *tcc*.
...
source file *octaspire_dern_amalgamated.c* can be found from the *etc*-directory of the source
distribution.
...
++++
<script type="text/javascript" src="https://asciinema.org/a/121995.js" id="asciicast-121995" async></script>
++++
...
To download the amalgamated source distribution and build Dern unit test runner and
Dern REPL in Raspberry Pi (Raspbian), Debian or Ubuntu (16.04 LTS) system:
...
Here is small example where the amalgamated source distribution is used as a Dern library in a program that wants to embed the Dern language:
...
----
// Compile with:
// gcc -O3 -std=c99 -Wall -Wextra main.c -Wl,-export-dynamic -ldl -lm
#define OCTASPIRE_DERN_AMALGAMATED_IMPLEMENTATION
#include "octaspire_dern_amalgamated.c"
...
The amalgamated source distribution is a single source code file, that contains everything that
is needed for using Dern. The same single file can be used to (by using different compiler flags):

- build a stand-alone unit test runner to test the file.
- build a stand-alone Dern REPL to run Dern programs interactively.
- build programs that want to embed the Dern programming language and use the
  amalgamated file as a single file library.

To use the amalgamated source distribution you don't need any build tools;
only C compiler is required.
...
To download the amalgamated source distribution and build Dern unit test runner and
Dern REPL in MINIX 3 (minix_R3.3.0-588a35b) system:
...
[source,bash]
----
curl -OkL https://github.com/octaspire/dern/releases/download/v0.73.0/octaspire_dern_amalgamated.c

clang -O3 -std=c99 -Wall -Wextra -DOCTASPIRE_DERN_AMALGAMATED_UNIT_TEST_IMPLEMENTATION \
                                 -DOCTASPIRE_DERN_CONFIG_BINARY_PLUGINS                \
                                 -DGREATEST_ENABLE_ANSI_COLORS                         \
octaspire_dern_amalgamated.c -Wl,-export-dynamic -lm -o octaspire-dern-test-runner

clang -O3 -std=c99 -Wall -Wextra -DOCTASPIRE_DERN_AMALGAMATED_REPL_IMPLEMENTATION \
                         -DOCTASPIRE_DERN_CONFIG_BINARY_PLUGINS                   \
octaspire_dern_amalgamated.c -Wl,-export-dynamic -lm -o octaspire-dern-repl
----

Here is small example where the amalgamated source distribution is used as a Dern library in a
program that wants to embed the Dern language:
...
----
// Compile with:
// clang -O3 -std=c99 -Wall -Wextra main.c -Wl,-export-dynamic -lm
#define OCTASPIRE_DERN_AMALGAMATED_IMPLEMENTATION
#include "octaspire_dern_amalgamated.c"
