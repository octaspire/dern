...
have the Emacs dotfiles from octaspire_dotfiles in use in your system (the submodule for
the dotfiles is not used at the moment, but it should be later). The Emacs
dotfiles are used to set the correct settings and styling when building the documentation
...
Running =make= generates first the amalgamation (only, if the source files
...
yet by unit tests. =make release/documentation/dern-manual.html= builds
the documentation.
